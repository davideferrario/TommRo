...
#include "rtos/cmsisRTOS/V1/thread/os/freeRTOS/tommRoCMwTemplateRTOS_CMSIS_RTOS_OsFreeRTOS.h"
...
#include "rtos/cmsisRTOS/V1/thread/os/keilRTX/tommRoCMwTemplateRTOS_CMSIS_RTOS_OsKeilRTX.h"
...
static tommRoC_err_enum_t _delay_msec(const uint32_t msec) {
    ...
    if (__CMSIS_RTOS_OS_DELAY_SUCCESS_RESULT__ != osDelay(msec)) {
...
static uint32_t _get_uptime_msec_mul_factor(void) {
    return osKernelSysTick() * _uptime_factor;
}
#endif
...
static tommRoC_err_enum_t _is_from_isr(bool* const isFromISRPtr) {
    *isFromISRPtr = (bool) xPortInIsrContext();
    return TOMMRO_C_ERR_SUCCESS;
}
#endif
