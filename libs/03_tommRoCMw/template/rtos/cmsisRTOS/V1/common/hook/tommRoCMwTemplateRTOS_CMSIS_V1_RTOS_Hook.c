 ...
 * includes
 ******************************************************************************/
#include "tommRoCMwTemplateRTOS_CMSIS_V1_RTOS_Hook.h"
 ...
 * local vars
 ******************************************************************************/
#if defined (__TOMMROC_MW_TEMPLATE_RTOS_CMSIS_RTOS_CHECK_FOR_STACK_OVERFLOW_IS_USED__) || defined (__TOMMROC_MW_TEMPLATE_RTOS_CMSIS_RTOS_MALLOC_FAILED_HOOK_IS_USED__)
#if defined (__TOMMROC_DEBUG_ENABLE__)
static const char __DEBUG_OBJ_ID__[] = "CMSISV1Hook";
#endif
#endif
...
void vApplicationStackOverflowHook( TaskHandle_t xTask,
    ...
    if (!_stack_overflow_has_been_detected) {
...
#if defined (__TOMMROC_MW_TEMPLATE_RTOS_CMSIS_RTOS_CHECK_FOR_STACK_OVERFLOW_IS_USED__)
    if (tommRoCMwTemplateRTOS_CMSIS_RTOS_HookStackOverflowHasBeenDetected()) {
        TOMMROC_DEBUG_PRINTLN_PLUS_STRING(TOMMRO_C_DEBUG_E_LEVEL, "TASK OVERFLOW!!!!!! ", _stack_overflow_task_name);
    }
#endif

#if defined (__TOMMROC_MW_TEMPLATE_RTOS_CMSIS_RTOS_MALLOC_FAILED_HOOK_IS_USED__)
    if (tommRoCMwTemplateRTOS_CMSIS_RTOS_HookMallocFailedHasBeenDetected()) {
