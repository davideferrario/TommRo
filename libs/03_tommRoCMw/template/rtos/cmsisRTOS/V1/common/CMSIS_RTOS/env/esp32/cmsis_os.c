...
#if !defined (inHandlerMode)
#define inHandlerMode                               xPortInIsrContext
#endif
...
osThreadId osThreadCreate (const osThreadDef_t *thread_def, void *argument)
{
  ...
  if((thread_def->buffer != NULL) && (thread_def->controlblock != NULL)) {
    handle = xTaskCreateStatic((TaskFunction_t)thread_def->pthread,(const char* const)thread_def->name,
              thread_def->stacksize, argument, makeFreeRtosPriority(thread_def->tpriority),
              (StackType_t*) thread_def->buffer, thread_def->controlblock);
  }
  else {
    if (xTaskCreate((TaskFunction_t)thread_def->pthread,(const char* const)thread_def->name,
              thread_def->stacksize, argument, makeFreeRtosPriority(thread_def->tpriority),
              &handle) != pdPASS)  {
      return NULL;
    }
...
{
  // NOTE: call delay with 0 msec as param to ensure a yield is done
  osDelay(0);
