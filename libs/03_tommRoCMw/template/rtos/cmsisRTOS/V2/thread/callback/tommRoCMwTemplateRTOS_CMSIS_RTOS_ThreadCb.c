 ...
 * local functions
 ******************************************************************************/
static osPriority_t _convert_priority(const tommRoC_mw_thread_priority_enum_t priority) {
...
tommRoC_mw_err_enum_t tommRoCMwTemplateRTOS_CMSIS_RTOS_ThreadCb_Create(
    ...
    TOMMROC_CTA_SIZE_OF_EQUALS(osThreadAttr_t,  ((tommRoC_mw_template_thread_t*)NULL)->osThreadAttr);
    TOMMROC_CTA_SIZE_OF_EQUALS(osThreadId_t,    ((tommRoC_mw_template_thread_t*)NULL)->osThreadId);
    ...
    TOMMROC_ASSERT_TOMMROC_LIB_FUNCT_SUCCESS(tommRoCUtilMemSet(&threadPtr->osThreadAttr, 0x00, sizeof(threadPtr->osThreadAttr)));
    ...
    threadPtr->osThreadAttr.name = (char*) ((NULL != threadNameStrPtr) ? (threadNameStrPtr) : (defaultThreadNameStr));
#endif

    threadPtr->osThreadAttr.priority    = priority;
#if defined (TOMMRO_C_ENV_ENVIRONMENT_IS_ESP32)
    threadPtr->osThreadAttr.stack_size  = stackSize * sizeof(uint16_t) * sizeof(uint32_t);
#else
    /**
     * NOTE:
     * In FreeRTOS V2 stack is not in bytes, but in sizeof(StackType_t) which is 4 on ARM ports.
     * In low level functions, the passed parameter will be divided by sizeof(StackType_t)
     */
    threadPtr->osThreadAttr.stack_size  = stackSize * sizeof(StackType_t);
#endif

    threadPtr->osThreadId = osThreadNew((osThreadFunc_t) mainThread, (void*) mainThreadArgPtr, &threadPtr->osThreadAttr);

    if (__CMSIS_RTOS_THREAD_NEW_ON_ERROR_RESULT__ == threadPtr->osThreadId) {
...
tommRoC_mw_err_enum_t tommRoCMwTemplateRTOS_CMSIS_RTOS_ThreadCb_GetID(
    ...
    TOMMROC_CTA_SIZE_OF_EQUALS(osThreadId_t, tommRoC_mw_template_thread_id_t);
    ...
    TOMMROC_ASSERT_TOMMROC_LIB_FUNCT_SUCCESS(tommRoCUtilMemCopySafe(threadIDPtr, sizeof(tommRoC_mw_template_thread_id_t), &res, sizeof(osThreadId_t), sizeof(res)));
