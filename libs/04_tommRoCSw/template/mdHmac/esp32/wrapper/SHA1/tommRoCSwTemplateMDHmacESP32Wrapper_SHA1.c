 ...
 * includes
 ******************************************************************************/
#include "tommRoCSwTemplateMDHmacESP32Wrapper_SHA1.h"
...
tommRoC_sw_err_enum_t tommRoCSwTemplateMDHmacESP32Wrapper_SHA1_Init(mbedtls_md_context_t* const mdCtxPtr) {
...
tommRoC_sw_err_enum_t tommRoCSwTemplateMDHmacESP32Wrapper_SHA1_Close(void) {
...
tommRoC_err_enum_t tommRoCSwTemplateMDHmacESP32Wrapper_SHA1_Perform(
        const   void*       const   keyPtr,
        const   uint16_t            keySize,
        const   void*       const   inputBufferPtr,
        const   uint16_t            inputBufferLen,
...
#if (MBEDTLS_VERSION_MAJOR == 2)
    static const mbedtls_md_info_t* mdInfoPtr = &mbedtls_sha1_info;
#elif (MBEDTLS_VERSION_MAJOR == 3)
    ...
    mdInfoPtr = mbedtls_md_info_from_type(MBEDTLS_MD_SHA1);
#endif
...
#if (MBEDTLS_VERSION_MAJOR == 2)
    TOMMROC_ASSERT_EQUALS(mbedtls_sha1_info.block_size, 64);
#endif

    // NOTE: used mbedtls_calloc but if static memory is used, the alloc will use that reserved memory
    paramMdCtxPtr   = mbedtls_calloc(1, sizeof(mbedtls_sha1_context));
#if (MBEDTLS_VERSION_MAJOR == 2)
    paramHmacCtxPtr = mbedtls_calloc(1, 2 * mbedtls_sha1_info.block_size);
#elif (MBEDTLS_VERSION_MAJOR == 3)
