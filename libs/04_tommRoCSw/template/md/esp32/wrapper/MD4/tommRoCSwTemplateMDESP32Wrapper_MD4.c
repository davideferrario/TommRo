 ...
 * includes
 ******************************************************************************/
#include "tommRoCSwTemplateMDESP32Wrapper_MD4.h"
 ...
 * local vars
 ******************************************************************************/
static mbedtls_md4_context* _md_ctx_ptr = NULL;

/*******************************************************************************
 * local functions
 ******************************************************************************/
static tommRoC_err_enum_t _perform(
                mbedtls_md4_context*    const   mdCtxPtr,
    ...
    res = mbedtls_md4_starts_ret(mdCtxPtr);

    if (0 != res) {
        return TOMMRO_C_LOW_LEVEL_ERR;
    }

    res = mbedtls_md4_update_ret(mdCtxPtr, (const unsigned char*) inputBufferPtr, (size_t) inputBufferLen);

    if (0 != res) {
        return TOMMRO_C_LOW_LEVEL_ERR;
    }

    res = mbedtls_md4_finish_ret(mdCtxPtr, (unsigned char*) outputBufferPtr);
...
tommRoC_err_enum_t tommRoCSwTemplateMDESP32Wrapper_MD4_MD4Perform(
        const   void*       const   inputBufferPtr,
        const   uint16_t            inputBufferLen,
                void*       const   outputBufferPtr) {

    mbedtls_md4_context* mdCtxPtr;
    tommRoC_err_enum_t res;

    mdCtxPtr = _md_ctx_ptr;

    mbedtls_md4_init(mdCtxPtr);
