...
#ifndef TOMMROC_HW_TEMPLATE_BUS_SPI_NXP_LOCATION_H_
#define TOMMROC_HW_TEMPLATE_BUS_SPI_NXP_LOCATION_H_
...
TOMMRO_C_UTIL_PACKED_PRE typedef struct TOMMRO_C_UTIL_PACKED_POST {
    ...
    PORT_Type*          gpioSCKPort;
    uint32_t            gpioSCKPin;
    port_mux_t          gpioSCKPortMux;

    PORT_Type*          gpioMISOPort;
    uint32_t            gpioMISOPin;
    port_mux_t          gpioMISOPortMux;

    PORT_Type*          gpioMOSIPort;
    uint32_t            gpioMOSIPin;
    port_mux_t          gpioMOSIPortMux;
    ...
    uint8_t             sckPortIdx;
    uint8_t             sckPin;
    uint32_t            sckIOCONFunc;

    uint8_t             misoPortIdx;
    uint8_t             misoPin;
    uint32_t            misoIOCONFunc;

    uint8_t             mosiPortIdx;
    uint8_t             mosiPin;
    uint32_t            mosiIOCONFunc;
...
#if defined (FSL_FEATURE_PORT_PCR_MUX_WIDTH) && FSL_FEATURE_PORT_PCR_MUX_WIDTH
    GPIO_Type*          gpioSCKGpio;
    GPIO_Type*          gpioMISOGpio;
    GPIO_Type*          gpioMOSIGpio;
...
#elif (MAKE_VERSION(2, 0, 1) == FSL_PORT_DRIVER_VERSION) || (MAKE_VERSION(2, 0, 2) == FSL_PORT_DRIVER_VERSION)
    gpio_port_num_t     gpioSCKPort;
    uint8_t             gpioSCKPin;
    gpio_port_num_t     gpioMISOPort;
    uint8_t             gpioMISOPin;
    gpio_port_num_t     gpioMOSIPort;
    uint8_t             gpioMOSIPin;
#endif

} tommRoC_hw_template_bus_spi_nxp_location_t;
