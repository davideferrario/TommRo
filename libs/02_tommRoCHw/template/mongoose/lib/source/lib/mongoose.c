...
__attribute__((weak)) int mg_http_upload(struct mg_connection *c, struct mg_http_message *hm,
                   const char *dir) {
...
__attribute__((weak)) char *mg_file_read(const char *path, size_t *sizep) {
  FILE *fp;
  char *data = NULL;
  size_t size = 0;
...
__attribute__((weak)) bool mg_file_write(const char *path, const void *buf, size_t len) {
  bool result = false;
  FILE *fp;
...
__attribute__((weak)) double mg_time(void) {
...
__attribute__((weak)) void mg_usleep(unsigned long usecs) {
#if MG_ARCH == MG_ARCH_WIN32
  Sleep(usecs / 1000);
...
__attribute__((weak)) unsigned long mg_millis(void) {
#if MG_ARCH == MG_ARCH_WIN32
  return GetTickCount();
